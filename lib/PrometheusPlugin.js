...
class PrometheusPlugin {
  ...
  async init(config, context) {
    ...
    this.hooks = {
      'connection:new': this.recordConnections('connection:new'),
      'connection:remove': this.recordConnections('connection:remove'),
  ...
  async syncRegisters() {
    ...
    do {
      const result = await this.context.accessors.sdk.ms.scan(cursor, {
        match: '{kuzzle_prometheus}-*'
      });
