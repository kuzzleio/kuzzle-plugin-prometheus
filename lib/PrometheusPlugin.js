...
class PrometheusPlugin {
  ...
  async init(config, context) {
    ...
    this.systemMetricsJob = Prometheus.collectDefaultMetrics({
      ...
      timeout: this.config.systemMetricsInterval || 5000
    });

    this.syncJob = setInterval(
      () => this.pushRegistry(),
      this.config.syncInterval || 7500
    );
  }
   ...
   * @param {String}  event
   */
  async recordRequests(request, event) {
  ...
  async syncRegisters() {
    ...
    const remoteRegisters =
      keys.length !== 0
        ? (await this.context.accessors.sdk.ms.mget(keys)).map(r => JSON.parse(r))
        : [];
